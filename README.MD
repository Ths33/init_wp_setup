...
```sh
./install/install.sh wp_
```

If no `DB_PREFIX` is provided, the script will use `wp_` as the default.
...
Se nenhum `DB_PREFIX` for fornecido, o script usará `wp_` como padrão.
